...
// Write a completely fresh HTML with clean ES5 JS (no template literals)
...
.stat{background:var(--card);border:1px solid var(--border);border-radius:16px;padding:24px;backdrop-filter:blur(10px);transition:border-color .2s}
...
.qr-card{background:var(--card);border:1px solid var(--border);border-radius:16px;padding:22px;backdrop-filter:blur(10px);display:grid;grid-template-columns:auto 1fr auto auto;gap:20px;align-items:center;transition:border-color .2s,transform .2s;animation:fadeIn .3s ease}
...
<div id="s-setup" class="auth-screen">
  <div class="auth-card">
    ...
    <button id="s-btn" class="btn btn-primary" onclick="doSetup()"><div class="spin"></div><span class="btn-text">Crear y entrar</span></button>
...
<div id="s-login" class="auth-screen hidden">
  <div class="auth-card">
    ...
    <button id="l-btn" class="btn btn-primary" onclick="doLogin()"><div class="spin"></div><span class="btn-text">Entrar</span></button>
...
<script>
var _token = sessionStorage.getItem("qr_t") || "";
var _editSlug = "";
var _delSlug = "";

function show(id) {
  var screens = ["s-setup", "s-login", "s-panel"];
...
function toast(msg, type) {
  ...
  _toastT = setTimeout(function() { t.classList.remove("show"); }, 3000);
}
...
async function init() {
  try {
    ...
    if (_token) {
      var t = await fetch("/api/qrs", { headers: { "Authorization": "Bearer " + _token } });
...
async function doSetup() {
  ...
  try {
    ...
    setLoad("s-btn", false);
    showErr("s-err", "Error de red: " + e.message);
  }
}
...
async function loginWith(pw) {
  ...
  try {
    ...
    if (d.token) {
      _token = d.token;
      sessionStorage.setItem("qr_t", _token);
      show("s-panel");
      loadQRs();
    } else {
      showErr("l-err", d.error || "Contrasena incorrecta");
      showErr("s-err", d.error || "Contrasena incorrecta");
    }
  } catch(e) {
    setLoad("l-btn", false); setLoad("s-btn", false);
    showErr("l-err", "Error de red: " + e.message);
  }
}

function logout() {
  _token = "";
  sessionStorage.removeItem("qr_t");
  show("s-login");
}

async function loadQRs() {
  try {
    var r = await fetch("/api/qrs", { headers: { "Authorization": "Bearer " + _token } });
    ...
    for (var i = 0; i < qrs.length; i++) {
      ts += (qrs[i].scans || 0);
      if (qrs[i].lastScan && (!last || qrs[i].lastScan > last)) last = qrs[i].lastScan;
    }
    ...
    list.innerHTML = "";
    for (var j = 0; j < qrs.length; j++) {
      (function(qr) {
        var card = document.createElement("div");
        card.className = "qr-card";
        var rurl = location.origin + "/r/" + qr.slug;
        var row = "<div class='qr-thumb'><canvas id='th-" + esc(qr.slug) + "'></canvas></div>";
        row += "<div class='qr-info'>";
        row += "<div class='qr-label'>" + esc(qr.label || qr.slug) + "</div>";
        row += "<div class='qr-slug' onclick=\"cp('" + rurl + "')\">&#128279; /r/" + esc(qr.slug) + "</div>";
        row += "<div class='qr-url'>&rarr; " + esc(qr.url) + "</div>";
        row += "</div>";
        row += "<div class='qr-scans'><div class='qr-scans-num'>" + (qr.scans || 0) + "</div><div class='qr-scans-label'>escaneos</div></div>";
        row += "<div class='qr-actions'>";
        row += "<button class='btn btn-ghost btn-sm' onclick=\"openQR('" + esc(qr.slug) + "','" + rurl + "')\">Ver QR</button>";
        row += "<button class='btn btn-ghost btn-sm' onclick=\"openEdit('" + esc(qr.slug) + "','" + esc(qr.url) + "','" + esc(qr.label || qr.slug) + "')\">Editar</button>";
        row += "<button class='btn btn-danger btn-sm' onclick=\"openDel('" + esc(qr.slug) + "')\">Borrar</button>";
        row += "</div>";
        card.innerHTML = row;
  ...
  } catch(e) { console.error("loadQRs error:", e); }
}
...
async function createQR() {
  ...
  setLoad("c-btn", true);
  try {
    var r = await fetch("/api/qrs", {
      method: "POST",
      headers: { "Content-Type": "application/json", "Authorization": "Bearer " + _token },
      body: JSON.stringify({ slug: slug, url: url, label: label })
    });
    var d = await r.json();
    setLoad("c-btn", false);
    if (r.ok) {
...
function openEdit(slug, url, label) {
  _editSlug = slug;
  document.getElementById("m-edit-slug").textContent = "/r/" + slug;
...
async function saveEdit() {
  setLoad("e-btn", true);
  try {
    var r = await fetch("/api/qrs/" + _editSlug, {
      method: "PUT",
      headers: { "Content-Type": "application/json", "Authorization": "Bearer " + _token },
      body: JSON.stringify({ url: document.getElementById("e-url").value, label: document.getElementById("e-label").value })
    });
    setLoad("e-btn", false);
...
function openDel(slug) {
  _delSlug = slug;
  document.getElementById("m-del-msg").textContent = "Eliminar el QR /r/" + slug + "? Esta accion no se puede deshacer.";
  openM("m-del");
}
...
async function confirmDel() {
  setLoad("d-btn", true);
  try {
    await fetch("/api/qrs/" + _delSlug, { method: "DELETE", headers: { "Authorization": "Bearer " + _token } });
    setLoad("d-btn", false);
    closeM("m-del"); toast("QR eliminado", "success"); loadQRs();
...
// Verify: decode back and check JS is intact
const decoded = Buffer.from(b64, "base64").toString("utf8");
const hasDoSetup = decoded.includes("function doSetup");
const hasDoLogin = decoded.includes("function doLogin");
console.log("doSetup present:", hasDoSetup);
console.log("doLogin present:", hasDoLogin);
console.log("HTML size:", html.length, "B64 size:", b64.length);

// Write the new worker.js using array join (no template literals in build output)
var lines = [
  "// free-dynamic-qr v5 - Cloudflare Worker",
  "// https://github.com/alfredgabriel/free-dynamic-qr",
  "// Paste ALL this code in the Cloudflare Workers editor.",
  "",
  ...
  "    if (!env.QR_KV) return json({ error: 'KV binding QR_KV not configured. Go to Worker Bindings, add KV namespace, variable name must be QR_KV.' }, 500);",
  "    try {",
...
fs.writeFileSync("C:/Users/alfre/Desktop/free-dynamic-qr/worker.js", final, "utf8");
console.log("worker.js written! Lines:", final.split("\n").length, "Size:", final.length);
